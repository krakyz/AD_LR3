...
behaviors:
  RollerBall: #
    trainer_type: ppo # тип обучения (PPO – Proximal Policy Optimization)
    hyperparameters:
      batch_size: 10 # количество опытов в каждой итерации градиентного спуска
      buffer_size: 100 #количество опытов, необходимое для обновления модели
      learning_rate: 3.0e-4 # начальная скорость обучения
      beta: 5.0e-4 # сила регуляризации энтропии (коэфициент случайности)
      epsilon: 0.2 # скорость изменения поведения
      lambd: 0.99 # параметр регуляризации, используемый при расчете обобщенной оценки преимущества (GAE – Generalized Advantage Estimation)
      num_epoch: 3 # количество проходов через буфер опыта при выполнении оптимизации градиентного спуска
      learning_rate_schedule: linear # параметр, определяющий изменение скорости обучения с течением времени
    network_settings: 
      normalize: false # параметр, определяющий, применяется ли нормализация к входным данным векторных наблюдений.
    ...
    reward_signals:
      extrinsic:
        gamma: 0.99 # фактор скидки для будущих вознаграждений (коэфициент поощрения)
        strength: 1.0 # коэффициент, на который умножается вознаграждение
    max_steps: 500000 # количество шагов, выполняемых в среде
    time_horizon: 64 # сколько шагов опыта необходимо собрать для каждого агента, прежде чем добавить его в буфер опыта
    summary_freq: 10000 # количество опытов, которое необходимо провести перед созданием и отображением статистики обучения
```

## Выводы
В ходе работы я познакомися с приницпом работы ML-агента, применил ML-агент для настройки поведения объектов в Unity, и получил начальные знания в области машинного обучения и искуственного интеллекта.
